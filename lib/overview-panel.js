...
export default class OverviewPanel {
  ...
  needHelpDidClick() {
    shell.openExternal(`${this.getApiEndpoint()}/freshdesk/login/${this.getUserToken()}`);
  }
  ...
  getApiEndpoint() {
    return atom.config.get('greyatom-ide').apiEndpoint
  }

  fetchTotalProgress() {
    const API_ENDPOINT = `${this.getApiEndpoint()}/user/dashboard/summary`
  ...
  fetchWeeklyProgress() {
    const API_ENDPOINT = `${this.getApiEndpoint()}/user/dashboard/weekly-summary`
    return fetch(API_ENDPOINT, { 
      headers: this.getHeader()
    })
  }
